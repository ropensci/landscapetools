...
show_shareplot <- function(
  ...
  if (is.null(multibuffer_df)) {
    result <- util_extract_multibuffer(
      landscape,
      points,
  ...
  p1 <- ggplot2::ggplot(
    result,
    ggplot2::aes(buffer, freq, group = layer, fill = layer)
  ) +
    ggplot2::geom_area(position = "fill") +
    ggplot2::facet_wrap(~id) +
    # ggplot2::expand_limits() +
    ggplot2::scale_y_continuous(
      name = "Shared proportion of landcover classes (%)",
      expand = c(0.01, 0.01),
    ...
    ggplot2::theme(
      ...
      strip.text = ggplot2::element_text(hjust = 0, size = 12, face = "plain"),
